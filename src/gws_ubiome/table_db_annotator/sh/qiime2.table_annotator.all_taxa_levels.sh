...
cat $rawTaxTable | perl $ratioCalc - > absolute_abundace_file.tsv ;

# create metadata files and manifest file compatible with qiime2 env and gencovery env

perl $perlScript $annotationDb $rawTaxTable $taxDbType $taxLevel > taxa_found.header_with_tag.relative.tsv ;

perl $perlScript $annotationDb absolute_abundace_file.tsv $taxDbType $taxLevel > taxa_found.header_with_tag.tsv ;
