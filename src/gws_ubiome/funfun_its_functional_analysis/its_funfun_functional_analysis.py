...
class FunfunFunctionalAnalysis(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if input_file.path.lower().endswith(".qza"):
            ...
            biom_file = os.path.join(shell_proxy_qiime2.working_dir, "feature-table.biom")

            # Add biom convert command
            cmd_biom_convert = f'biom convert -i {biom_file} -o {biom_file.replace(".biom", ".tsv")} --to-tsv'
            res_biom_convert = shell_proxy_qiime2.run(cmd_biom_convert, shell_mode=True)
        ...
        # Now, apply funfun
        shell_proxy_funfun = FunfunShellProxyHelper.create_proxy(self.message_dispatcher)
        output_folder = os.path.join(shell_proxy_funfun.working_dir, 'funfun_output_folder')
