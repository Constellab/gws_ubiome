...
class Qiime2TaxonomyDiversityNCBIExtractor(Task):
    """
    Qiime2TaxonomyDiversityNCBIExtractor class.

    This task classifies reads by taxon using a pre-fitted sklearn-based taxonomy classifier. By default, we suggest a pre-fitted Naive Bayes classifier for the database NCBI 16S (from 2022.07.12) with reference full-length sequences clustered at 99% sequence similarity.

    **Minimum required configuration:** Digital lab SC2

    **About NCBI:**
    NCBI 16S RNA database is a collection of 16S ribosomal RNA gene sequences from various organisms stored in the National Center for Biotechnology Information (NCBI) database.
