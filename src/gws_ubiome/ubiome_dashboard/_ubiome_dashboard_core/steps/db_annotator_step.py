...
def dialog_db_annotator_params(ubiome_state: State):
    ...
    st.text_input(
        translate_service.translate("taxa_composition_scenario_name"),
        placeholder=translate_service.translate("enter_taxa_composition_name"),
        value=f"{ubiome_state.get_current_analysis_name()} - Taxa Composition",
        key=ubiome_state.DB_ANNOTATOR_SCENARIO_NAME_INPUT_KEY,
    )
    ...
    with col1:
        save_clicked = st.button(
            translate_service.translate("save_taxa_composition"),
            width="stretch",
            icon=":material/save:",
            key="button_db_annotator_save",
        )

    with col2:
        run_clicked = st.button(
            translate_service.translate("run_taxa_composition"),
            width="stretch",
            icon=":material/play_arrow:",
            key="button_db_annotator_run",
        )
    ...
    if save_clicked or run_clicked:
        ...
        scenario = create_base_scenario_with_tags(
            ...
            ubiome_state.get_scenario_user_name(ubiome_state.DB_ANNOTATOR_SCENARIO_NAME_INPUT_KEY),
        )
        ...
        taxonomy_folder_output = protocol_proxy_tax.get_process("taxonomy_process").get_output(
            "result_folder"
        )

        # Add input resources
        taxonomy_folder_resource = protocol.add_process(
            InputTask,
            "taxonomy_folder_resource",
            {InputTask.config_name: taxonomy_folder_output.get_model_id()},
        )

        annotation_table_resource = protocol.add_process(
            InputTask,
        ...
        protocol.add_connector(
            out_port=taxonomy_folder_resource >> "resource",
            in_port=db_annotator_process << "diversity_folder",
        )
        protocol.add_connector(
            out_port=annotation_table_resource >> "resource",
...
def render_db_annotator_step(selected_scenario: Scenario, ubiome_state: State) -> None:
    ...
    if not selected_scenario:
        if not ubiome_state.get_is_standalone():
            ...
            st.button(
                translate_service.translate("configure_new_taxa_composition_scenario"),
                icon=":material/edit:",
                width="content",
        ...
        render_scenario_table(
            list_scenario_db_annotator, "db_annotator_process", "db_annotator_grid", ubiome_state
        )
        ...
        tab_relative, tab_absolute = st.tabs(
            [
                translate_service.translate("relative_abundance"),
                translate_service.translate("absolute_count"),
            ]
        )
        ...
        with tab_relative:
            ...
            relative_table_output = protocol_proxy.get_process("db_annotator_process").get_output(
            ...
            relative_plot_output = protocol_proxy.get_process("db_annotator_process").get_output(
                "relative_abundance_plotly_resource"
            )
        ...
        with tab_absolute:
            ...
            absolute_table_output = protocol_proxy.get_process("db_annotator_process").get_output(
                "absolute_abundance_table"
            )
