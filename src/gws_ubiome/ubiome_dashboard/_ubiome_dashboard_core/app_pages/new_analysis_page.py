    ...
    with StreamlitContainers.container_full_min_height(
        ...
        with st.form(clear_on_submit=False, enter_to_submit=True, key="new_analysis_form"):
            ...
            resource_select.select_resource(
                ...
                key=ubiome_state.RESOURCE_SELECTOR_FASTQ_KEY,
                default_resource=None,
            )

            form_config = StreamlitTaskRunner(Qiime2MetadataTableMaker)
            form_config.generate_config_form_without_run(
                session_state_key=ubiome_state.QIIME2_METADATA_CONFIG_KEY,
            ...
            with cols[0]:
                st.text_input(
                    ...
                    key=ubiome_state.ANALYSIS_NAME_USER,
                )
            ...
            if submit_button:
                ...
                sequencing_type = ubiome_state.get_qiime2_metadata_config()["config"].get(
                    "sequencing_type"
                )
                ...
                scenario.add_tag(
                    Tag(
                        ubiome_state.TAG_BRICK,
                        ubiome_state.TAG_UBIOME,
                        is_propagable=False,
                        auto_parse=True,
                    )
                )
                scenario.add_tag(
                    Tag(ubiome_state.TAG_UBIOME, ubiome_state.TAG_METADATA, is_propagable=False)
                )
                scenario.add_tag(
                    Tag(
                ...
                metadata_process: ProcessProxy = protocol.add_process(
                    Qiime2MetadataTableMaker,
                    "metadata_process",
                    config_params=ubiome_state.get_qiime2_metadata_config()["config"],
                )
                protocol.add_connector(
                    out_port=fastq_resource >> "resource",
                    in_port=metadata_process << "fastq_folder",
                )
                # Add output
                protocol.add_output(
                    "metadata_process_output",
                    metadata_process >> "metadata_table",
                    flag_resource=False,
                )
