...
class ItsPicrust2FunctionalAnalysis(Task):
    ...
    - <b style="margin-left: 2.5em"> ec_ITS_counts.txt_metagenome_out </b> : Folder containing unstratified EC number metagenome predictions.
      (pred_metagenome_unstrat.tsv.gz), sequence table normalized by predicted ITS copy number abundances.
      (seqtab_norm.tsv.gz), and the per-sample NSTI values weighted by the abundance of each ASV (weighted_nsti.tsv.gz).
    - <b style="margin-left: 2.5em"> pathways_out </b>      : Folder containing predicted pathway abundances and coverages per-sample, based on predicted EC number abundances stored in path_abun_unstrat.tsv.gz file.
    """
    ...
    python_file_path = os.path.join(
        ...
        "_its_picrust2_functional_analysis.py"
    )
